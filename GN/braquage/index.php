...
if (isset($_GET['mode']) && $_GET['mode'] === 'export_view' && isset($_SESSION['role']) && $_SESSION['role'] === 'orga') {
    ...
    <head>
        ...
        <style>
            ...
            @media print { .no-print { display: none; } body { -webkit-print-color-adjust: exact; print-color-adjust: exact; padding: 0; } .timeline { border-color: #ccc; } }
        </style>
    </head>
    <body>
        <button onclick="window.print()" class="btn-print no-print">🖨️ IMPRIMER / PDF</button>
        <div class="header"><h1>BRAQUAGE</h1><div class="meta">Rapport de session<br>Date : <?php echo date('d/m/Y'); ?></div></div>
        <div class="timeline">
            <?php foreach($state['history'] as $h): $titre_from = $scenarios[$h['from']]['titre'] ?? 'Scène '.$h['from']; $titre_to = $scenarios[$h['to']]['titre'] ?? 'Scène '.$h['to']; ?>
            <div class="event">
                <div class="dot"></div><div class="time"><?php echo $h['time']; ?></div>
...
<head>
    ...
    <style>
        ...
        .header-title { font-weight: 800; font-size: 1.1em; letter-spacing: 0.5px; text-transform: uppercase; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
        .scene-badge { background: var(--primary); color: var(--btn-text); padding: 4px 10px; border-radius: 20px; font-size: 0.75em; font-weight: bold; margin-right: 10px; flex-shrink: 0; }
        ...
        /* CONTENT */
        .content { padding: 0 15px; margin: 0 auto; width: 100%; box-sizing: border-box; }
        ...
        @media (min-width: 1024px) {
            ...
            .orga-scene-grid {
                display: grid;
                grid-template-columns: 2fr 1fr; /* Main content left, MJ Info right */
                gap: 20px;
                align-items: start;
            }
            .mj-info-col {
        ...
        .btn-undo { background: transparent; border: 1px solid var(--primary); color: var(--primary); font-size: 0.85em; padding: 10px; justify-content: center; width: 60%; margin: 0 auto 25px auto; opacity: 0.7; }
        ...
        .theme-select { width: 100%; padding: 12px; border-radius: 8px; font-size: 1em; margin-bottom: 20px; cursor: pointer; background: var(--bg); color: var(--text); border: 1px solid var(--border); appearance: none; -webkit-appearance: none; background-image: url("data:image/svg+xml;charset=US-ASCII,%3Csvg%20xmlns%3D%22http%3A%2F%2Fwww.w3.org%2F2000%2Fsvg%22%20width%3D%22292.4%22%20height%3D%22292.4%22%3E%3Cpath%20fill%3D%22%23999%22%20d%3D%22M287%2069.4a17.6%2017.6%200%200%200-13-5.4H18.4c-5%200-9.3%201.8-12.9%205.4A17.6%2017.6%200%200%200%200%2082.2c0%205%201.8%209.3%205.4%2012.9l128%20127.9c3.6%203.6%207.8%205.4%2012.8%205.4s9.2-1.8%2012.8-5.4L287%2095c3.5-3.5%205.4-7.8%205.4-12.8%200-5-1.9-9.2-5.5-12.8z%22%2F%3E%3C%2Fsvg%3E"); background-repeat: no-repeat; background-position: right 15px top 50%; background-size: 12px auto; }
...
<?php else: ?>
    ...
    <header>
        <div style="display:flex; align-items:center;">
            ...
            <div class="header-title"><?php echo $current_scene['titre'] ?? 'FIN'; ?></div>
        </div>
                ...
                <div class="orga-scene-grid">
                    ...
                    <div class="main-col">
                        ...
                        <div style="margin-top: 30px; border-top: 1px solid var(--border); padding-top: 20px;">
                            ...
                            <?php else: ?>
                                <div class="card">🚫 Fin du scénario ou pas de choix.</div>
                            <?php endif; ?>
                        </div>
                    </div>

                    <div class="mj-info-col">
    ...
    <div class="drawer" id="drawer">
        ...
        <div class="drawer-content">
            <div id="menu-main" class="submenu active">
                ...
                <div style="margin-bottom:20px;">
                    <span class="card-label">🎨 Thème</span>
                    <form method="GET">
            ...
            <div id="menu-jump" class="submenu">
                <button class="btn btn-secondary back-main" style="margin-bottom:20px; justify-content:flex-start;"><i class="fa-solid fa-arrow-left"></i> &nbsp; Retour</button>
                <h3 style="color:var(--text-muted);">Navigation</h3>
                <form method="POST">
                ...
                <div class="timeline">
                    ...
                    foreach($preview as $h): ?>
                        <div class="timeline-item">
                            <div class="t-time"><?php echo $h['time']; ?></div>
                            <div class="t-scene">Scène <?php echo $h['to']; ?></div>
                        </div>
                    <?php endforeach; ?>
                </div>
            </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelectorAll('.btn-menu').forEach(btn => {
        btn.addEventListener('click', function() { showSubMenu(this.getAttribute('data-submenu')); });
    });
    document.querySelectorAll('.back-main').forEach(btn => {
        btn.addEventListener('click', function() { showSubMenu('menu-main'); });
    });
});
</script>
